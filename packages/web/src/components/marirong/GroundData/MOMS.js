                                            ...
                                            <Select
                                                ...
                                                onChange={(event) => {
                                                    const { value } =
                                                        event.target;
                                                    console.log(
                                                        "options.find((e) => e.feature_id === value)",
                                                        options.find(
                                                            (e) =>
                                                                e.feature_id ===
                                                                value,
...
export default function MOMS() {
    ...
    const [observanceTs, setObservanceTS] = useState(
    ...
    const fetchLatestData = async () => {
        ...
        const instance_response = await MarGroundData.GetMomsInstancesPerSite(
            cookies.credentials.site_id,
        );
        ...
        if (features_response.status === true) {
            ...
            if (instance_response.status === true) {
                ...
                if (defaultStrValues.feature_id.length) {
                    console.log(
                        "defaultStrValues.feature_id",
                        defaultStrValues.feature_id,
                    );
                    setInstanceOptions(
                        feature_names_ref.current[defaultStrValues.feature_id],
                    );
                }
                const type_rows = features_response.data.map((feat, i) => (
                    <MenuItem value={parseInt(feat.feature_id)} key={i}>
                        {feat.feature_type}
                    </MenuItem>
                ));
            }
    ...
    const handleFeatureTypeChange = ({ target: { value } }) => {
        ...
        setMomsInstancesFormData({
            ...momsInstancesFormData,
            feature_id: value,
        });
        // Set Feature Name dropdown
    ...
    const handleEdit = (data) => {
        ...
        setDefaultStrValues({
            ...defaultStrValues,
            observance_ts: moment(data.observance_ts).format(
                "YYYY-MM-DD HH:mm:ss",
            ),
    ...
    const confirmDelete = async () => {
        ...
        if (response.status === true) {
            ...
            setNotifStatus("error");
            setNotifText(
                "Failed to delete moms data. Please contact the developers or file a bug report",
            );
        }
        setNotifText(response.message);
        setOpenNotif(true);
    ...
    const submitNewFeatureName = async (values) => {
        ...
        let json = values;
        json.reporter =
            cookies.credentials.firstname + " " + cookies.credentials.lastname;
    ...
    const handleUploadOpen = async (data) => {
        console.log(data);
        const response = await MarGroundData.FetchMOMSAttachments(
            parseInt(data.id),
        );
    ...
    return (
        <Fragment>
            ...
            >
                <DialogTitle id="form-dialog-title">
                    Manifestation of Movements {command} Entry
                </DialogTitle>
                <DialogContent>
                    <Formik
                                    ...
                                    <Grid container spacing={1}>
                                        ...
                                        <Grid item xs={6}>
                                            ...
                                            <Link
                                                ...
                                                onClick={() => {
                                                    console.log(
                                                        "Clicked add new feature type link",
                                                    );
                                                    setIsFeatureDialogOpen(
                                                        true,
                                                    );
                                                }}
                                            >
                                        ...
                                        <Grid item xs={6}>
                                            {/* Feature Name */}
                                            <FormControl fullWidth>
                                                <InputLabel htmlFor="name-native-simple">
                                                    Feature Name
                                                </InputLabel>
                                                <Select
                                                    value={
                                                        defaultStrValues.instance_id
                                                    }
                                                    onChange={
                                        ...
                                        <Grid item xs={6}>
                                            ...
                                            <Link
                                                ...
                                                onClick={() => {
                                                    console.log(
                                                        "Clicked add new feature name link",
                                                    );
                                                    setIsInstanceDialogOpen(
                                                        true,
                                                    );
                                                }}
                                            >
            ...
            >
                ...
                <DialogContent>
                    <DialogContentText id="alert-dialog-description">
                        Removing this MOMS data{" "}
                        <strong>
                            <u>cannot</u>
                        </strong>{" "}
                        be undone. Are you sure you want to remove this entry?
                    </DialogContentText>
                </DialogContent>
                <DialogActions>
        ...
        /> */}
            ...
            <Dialog
                ...
                onClose={() => setIsInstanceDialogOpen(false)}
                aria-labelledby="form-dialog-title"
            >
                <DialogTitle id="form-dialog-title">
                    New Feature Name
