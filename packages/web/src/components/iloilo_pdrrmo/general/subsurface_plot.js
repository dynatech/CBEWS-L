...
import { getSubsurfacePlotData } from "../general/jsons/umi_sample_subsurface_data_not_final";
import { getSubsurfacePlotMarData } from "../general/jsons/mar_sample_subsurface_data_not_final";
...
function SubsurfacePlot(props) {
    ...
    if (site === "mar") {
        getSubsurfacePlotMarData.forEach(({ type, data }) => {
            let temp = [];
    ...
    let commands = [];
    if (feature === "data_analysis") {
        commands.push( <Grid container align="center" style={{ paddingBottom: 20 }}>
        <Grid item xs={3} />
        <Grid item xs={3}>
